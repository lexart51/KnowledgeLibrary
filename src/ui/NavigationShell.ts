...
export function renderKnowledgeNavigation(parent: HTMLElement, plugin: KnowledgeLibraryPlugin, active: KnowledgeNavigationActiveView): HTMLElement {
  ...
  navButton(nav, "Topics", "Open Topics", active === "topics", () => void plugin.openTopicPicker());
  ...
  navButton(nav, "Add Resource", "Add Resource", active === "add", () => plugin.openAddResourceModal());
  navButton(nav, "Settings", "Open Knowledge Library settings", active === "settings", () => plugin.openSettings());
  return nav;
}

function navButton(parent: HTMLElement, label: string, title: string, active: boolean, onClick: () => void): void {
  const button = parent.createEl("button", {
    text: label,
    cls: `knowledge-library-shell-nav-button${active ? " is-active" : ""}`,
    attr: {
      "aria-label": title,
