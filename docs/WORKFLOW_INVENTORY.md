...
3. ~~No cross-surface "recent items" or "continue studying" view exists today outside of Library's Progress filter and Sort-by-updated, both of which require already being in Library with the right filter set.~~ **Fixed** — Dashboard now has a "Continue Learning" section (in-progress, non-completed resources, most recently updated first, click to open the note directly). See `LESSONS_LEARNED.md`, "Continue Learning Added." This was scoped deliberately narrow: no Home screen, no Topics, no navigation shell — just a data section added to the existing Dashboard view.
...
This inventory intentionally stops here for gap 4. Per the brief, Phase 2 (information architecture) and Phase 3 (wireframes) come next for that gap, and are separate, later decisions — not implied by anything in this document.
