...
- The fix is not the same for both affected views. Dashboard's container (`.knowledge-library-dashboard`) had no competing design constraint, so adding `overflow-y: auto; overflow-x: hidden; height: 100%; min-height: 0;` to it directly, isolated into its own rule (it originally shared a declaration block with unrelated selectors `.knowledge-library-progress-field`, `.knowledge-library-editor-form`, `.knowledge-library-relationships-section`, `.knowledge-library-management-list`, which must not receive the same change), was built, passed all 168 existing tests, and was deployed to the isolated alpha.6 test vault for live confirmation.
- Library's container (`.knowledge-library-view`) cannot use the same fix. It intentionally uses `overflow: visible` so a separate dedicated child, `.knowledge-library-scroll`, handles scrolling for the card grid alone while the header/toolbar stay fixed — a real, already-tested design (`tests/library-view.test.ts` asserts `overflow: visible` on this selector). The nav bar renders as a sibling of `.knowledge-library-scroll`, not inside it, so it ends up sticky relative to Obsidian's outer container rather than the vault's own scroll region. Fixing Library correctly means moving the nav bar's render call inside `.knowledge-library-scroll`, which is real implementation work, not a CSS-only patch, and was not attempted here.
