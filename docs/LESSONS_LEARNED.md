...
Separately, a real (not hypothetical) three-way install was set up and tested: `knowledge-library-v6-obsidianvault-test`, a distinct plugin id installed directly in `Obsidian_Vault` with its own connectors pointing back at `YouTubes` and `_Docs`. It works — full local features, its own index, its own resource folder. The trade-off observed firsthand: three independent "active vault" data stores with no reconciliation between them, so the same item can end up tracked differently (favorite/progress/collection) depending on which vault's instance touched it last. Standing recommendation given that trade-off: keep `YouTubes` as the one hub; this test install is optional and not part of the recommended setup. Because it lives under `D:\Dropbox\Obsidian_Vault`, it is Dropbox-synced and will appear on any machine (including a future PINHA setup) that opens that vault — worth remembering if it's ever removed, since removal must happen at the source (delete `D:\Dropbox\Obsidian_Vault\.obsidian\plugins\knowledge-library-v6-obsidianvault-test\` and its entry in that vault's `community-plugins.json`), not per-machine.
...
- **Not Dropbox-synced, local to PAPAIA only**: `C:\ObsidianTestVaults\` (isolated test vaults, scratch build copy, synthetic-data generator, one-off migration scripts). Nothing here is unique or required — the test vaults hold synthetic/disposable data, the scratch build is a disposable rebuild of what's already in git, and the one-off scripts were single-use. All of it is deliberately outside Dropbox so testing never risks production data; none of it needs to survive a PAPAIA failure.
